...
public class AddDatabases implements InteractionCommand {
    ...
    public void execute() {
		 ...
		 * LOGGER.error("Unexpected unregistered MediatorGui.treeDatabase() in "+
		 * this.getClass()); }
		 */
		 ...
		 * MediatorGui.treeDatabase().getModel();
		 * 
		 * // First node in tree DefaultMutableTreeNode root = (DefaultMutableTreeNode)
		 * treeModel.getRoot();
		 * 
		 * // Loop into the list of databases for (Database database: this.databases) {
		 * // Create a node model with the database element AbstractNodeModel
		 * newTreeNodeModel = new NodeModelDatabase(database); // Create the node
		 * DefaultMutableTreeNode newNode = new
		 * DefaultMutableTreeNode(newTreeNodeModel); // Save the node
		 * MediatorGui.frame().getTreeNodeModels().put(database, newNode); // Add the
		 * node to the tree root.add(newNode); }
		 * 
		 * // Refresh the tree treeModel.reload(root); // Open the root node
